...
def decompress_gz(input_path: str, output_dir: str) -> str:
    ...
    if result.returncode != 0:
        ...
        if stderr:
            print(f"[sdb] gzip error: {stderr.strip()}")
        if temp_path.exists():
            temp_path.unlink()
        raise subprocess.CalledProcessError(
...
def decompress_xz(input_path: str, output_dir: str) -> str:
    ...
    if result.returncode != 0:
        stderr = result.stderr.decode('utf-8', errors='replace') if result.stderr else ''
        if stderr:
            print(f"[sdb] xz error: {stderr.strip()}")
        if temp_path.exists():
            temp_path.unlink()
        raise subprocess.CalledProcessError(
            result.returncode, cmd, stderr=stderr
...
def decompress_tar_gz(input_path: str, output_dir: str) -> str:
    ...
    print(f"[sdb] Extracting {input_path.name}")
    ...
    if result.returncode != 0:
        if result.stderr:
            print(f"[sdb] tar error: {result.stderr.strip()}")
        raise subprocess.CalledProcessError(
            result.returncode, cmd, output=result.stdout, stderr=result.stderr
        )

    print(f"[sdb] Extracted: {input_path.name} -> {output_dir}")
